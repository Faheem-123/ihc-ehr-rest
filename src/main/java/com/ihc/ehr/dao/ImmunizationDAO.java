...
public interface ImmunizationDAO {
	...
	WrapperImmRegEvaluationHistoryForecastMessageDetails getImmRegEvaluationHistoryForecastMessageDetails(String registryCode,Long messageId);

	ServiceResponse<HL7FileGenerationResult> generateVXUFile(
			ImmRegVXUCriteria immRegVXUCriteria);

	
	ServiceResponse<HL7FileGenerationResult> generateQBPFile(
			ImmRegQBPCriteria immRegQBPCriteria);


	ServiceResponse<ORMKeyValue> ProcessAcknowledgementMessageFromFileData(MultipartFile responseFileData, SearchCriteria searchCriteria);
